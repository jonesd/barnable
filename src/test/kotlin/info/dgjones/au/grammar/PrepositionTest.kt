...
class PrepositionTest {
    ...
    fun `With preposition matches human`() {
        ...
        assertEquals("Fred", lunch.value(MopMealFields.EATER_A)?.valueName(Human.FIRST_NAME))
        assertEquals("George", lunch.value(MopMealFields.EATER_B)?.valueName(Human.FIRST_NAME))
    }
